 ...
 * Restore. A "Save snapshot" button creates a manual commit.
 *
 * adr: adr/0006-document-history-attribution.md · adr: adr/right-rail.md
 */
...
export default function VersionsTab({ docId }: RightRailTabProps) {
  ...
  const handleSaveVersion = useCallback(async () => {
    ...
    const note = window.prompt('Save snapshot — optional note (e.g. "first full draft"):') ?? undefined;
    setBusy(true);
    try {
      ...
      <div className="versions-tab__list">
        {commits.length === 0 ? (
          <div className="versions-tab__empty">
            No snapshots yet. A snapshot is saved when an agent finishes writing,
            when you accept changes, or when you hit "Save snapshot."
          </div>
        ) : (
          commits.map((c) => (
            <div
              ...
              {expanded === c.ts && (
                <div className="commit-row__detail" onClick={(e) => e.stopPropagation()}>
                  ...
                  <button
                    ...
                    title={c.restorable ? 'Restore the document to this snapshot' : 'Snapshot pruned — cannot restore'}
