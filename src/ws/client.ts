...
interface UseWebSocketOptions {
  ...
  /** The Library shelf changed (mirror write, adopt, restore) — re-fetch the
   *  listing. The Library section is a view over the shelf, so it refreshes
   *  with the docs tree. */
  ...
  /** Called whenever the server reports the set of in-progress write filenames. */
  onPendingFilenamesChanged?: (filenames: Set<string>) => void;
  /** Live progress line from an in-flight chat turn. */
  onChatProgress?: (payload: { docId: string; sessionId: string; line: string }) => void;
  /** Called on reconnect so the app can re-sync editor state to server */
  getEditorState?: () => { document: any } | null;
}

export function useWebSocket({ onNodeChanges, onAgentStatus, onDocumentSwitched, onDocumentReloaded, onDocumentsChanged, onWorkspacesChanged, onLibraryChanged, onTitleChanged, onPendingDocsChanged, onMetadataChanged, onSyncStatus, onWritingStarted, onWritingFinished, onIdRewrites, onPendingFilenamesChanged, onChatProgress, getEditorState }: UseWebSocketOptions) {
  const wsRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  // Last agent-write version seen from the server.
  const docVersionRef = useRef<number>(0);
  // Live set of filenames the server currently reports as in-progress writes.
    ...
    function connect() {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'chat-progress' && typeof msg.line === 'string') {
            onChatProgressRef.current?.({
              docId: typeof msg.docId === 'string' ? msg.docId : '',
