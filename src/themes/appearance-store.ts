...
function migrateIfNeeded(): void {
  ...
  // M4e: Outline became the default canvas. applyAppearance used to write the
  // then-default 'seamless' to storage on every boot, so a stored 'seamless'
  // may be that artifact rather than a choice — drop it once, behind a flag,
  // so the new default applies. A seamless picked after this migration is a
  // deliberate choice and survives.
  if (!localStorage.getItem('ow-canvas-m4e')) {
    if (localStorage.getItem(KEYS.canvas) === 'seamless') localStorage.removeItem(KEYS.canvas);
    localStorage.setItem('ow-canvas-m4e', '1');
  }
}
