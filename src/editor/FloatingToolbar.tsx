...
import { collectSelectionQuote } from './selection-quote';
...
export default function FloatingToolbar({ editor, onDiscuss }: { editor: Editor; onDiscuss?: (quote: { text: string; paraIds: string[] }) => void }) {
  ...
  useEffect(() => {
    const onTransaction = () => {
      ...
      const isNodeSel = !empty && 'node' in editor.state.selection && editor.state.selection.node;
  ...
  return (
    ...
    >
      ...
      {/* Discuss with agent — the writer's selection becomes a quote in the
        * chat composer (paraIds ride along as propose_edits anchors). */}
      <ToolbarButton
        onClick={() => {
          const quote = collectSelectionQuote(editor);
          if (quote) onDiscuss?.(quote);
        }}
