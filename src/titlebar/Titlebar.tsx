...
interface TitlebarProps {
  ...
  /** Save-status chip state from App's existing doc-update send flow. */
  saveState?: 'idle' | 'saving' | 'saved';
  lastSavedAt?: string | null;
  /** Pending rename staged by an agent, rendered in place of the title. */
  pendingTitle?: { from: string; to: string } | null;
  docId?: string;
}

/**
 * Titlebar. Ported from openwriter and adapted to writer-app's shell state.
 *
 * Right side of the titlebar — slight asymmetry vs. the left side:
 *   - rail OPEN: empty (HideRail + format toggle live inside the rail topbar)
 *   - rail CLOSED: format-toolbar toggle + OpenRail icon + save-status chip
 *
  ...
  return (
    <div className="titlebar">
      ...
      <div className="titlebar-center">
        {editing ? (
          <PendingTitleField pendingTitle={pendingTitle} docId={docId} baseClass="titlebar-input">
            <input
              ref={inputRef}
          ...
          <PendingTitleField pendingTitle={pendingTitle} docId={docId} baseClass="titlebar-title">
      ...
      <div className="titlebar-right">
        ...
        {!railVisible && (
          <button
            className="titlebar-nav-btn"
            onClick={() => openTab(activeTab || 'review')}
        ...
        <span
          className={`titlebar-save titlebar-save--${saveState}`}
          title={
            saveState === 'saving'
              ? 'Saving to your desk...'
              : saveState === 'saved'
                ? 'Saved — your desk has your latest words'
                : 'No changes yet'
          }
        >
          {chipLabel}
