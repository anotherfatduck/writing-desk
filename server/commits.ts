...
export function commitVersion(
  ...
  // `restorable` flag reflects this). adr: adr/0006-document-history-attribution.md
...
export function commitFromFile(
  ...
  const commit = commitVersion(docId, markdown, opts);
  // accept/agent-finished boundaries usually carry no new edit-events (an agent-finished commit often precedes the accept),
  // and seam #10's contract is "the boundary happened at ts", so the signal fires on attempt.
  if (opts.trigger === 'accept' || opts.trigger === 'agent-finished') {
    commitEvents.emit('commit', { docId, trigger: opts.trigger, ts: commit?.ts ?? opts.nowTs });
  }
  return commit;
}
 ...
 * (no new events → no-op). adr: adr/0006-document-history-attribution.md
 */
