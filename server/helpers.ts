...
// OW_HOME: per-writer data root (spike/deploy patch — upstream keeps ~/.openwriter).
// Read at module load; the CLI must be started with the env already set.
export const ROOT_DIR = process.env.OW_HOME
  ? resolve(process.env.OW_HOME)
...
/**
 * True when `p` names a path inside the read-only library shelf
 * (`<ROOT_DIR>/library/`) — the librarian's mirror area. Bare basenames
 * resolve to DATA_DIR and are never library paths. Canonicalizes both sides
 * (as `isExternalDoc` does) so `..` traversal, separator direction, and
 * symlink spellings classify consistently.
 *
 * adr: adr/0007-git-orchestrator-dumb-host-side.md
 */
export function isLibraryPath(p: string): boolean {
  if (!p) return false;
  if (!isAbsolute(p) && !/[/\\]/.test(p)) return false;
  const canonLib = canonicalizePath(join(ROOT_DIR, 'library'));
  const canonP = canonicalizePath(p);
  return canonP === canonLib || canonP.startsWith(canonLib + sep);
}

/**
 * Stable tag on the read-only refusal. Call sites that must not throw — the
 * debounced-save timer, whose callback escaping a throw is an
 * uncaughtException that bin/server.ts answers with process.exit(1) — match on
 * this instead of the message text.
 */
export const LIBRARY_READONLY_CODE = 'ELIBRARYREADONLY';

/**
 * One chokepoint for the library read-only rule (spec §3): every doc-write
 * entry point calls this before touching disk, so no path — HTTP, WS, MCP, or
 * a watcher-driven save — can write into the shelf. Adopt is the only write
 * path out of it. Throws `library area is read-only` (status 400) when the
 * target lands inside.
 */
export function assertWritableDocPath(p: string): void {
  if (isLibraryPath(p)) {
    throw Object.assign(new Error('library area is read-only'), {
      status: 400,
      code: LIBRARY_READONLY_CODE,
    });
  }
}

/** True for the refusal above — the one error a non-request context (the
 *  debounced-save timer) contains and reports instead of letting it escape. */
export function isLibraryReadOnlyError(err: unknown): boolean {
  return !!err && (err as { code?: unknown }).code === LIBRARY_READONLY_CODE;
}
