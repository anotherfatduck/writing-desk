 ...
 *
 * Blame is anchored to `sentenceHash`, the same content-addressed fingerprint
 * the enrichment path already computes every save (node-fingerprint.ts). A
 * sentence whose hash is unchanged keeps its author; a new hash is attributed
 * to the current actor; a removed sentence's author is retained in `retired`
 * so paste-back revives it.
 *
 * Storage tiers (per adr/0006-document-history-attribution.md):
 ...
 *
 * This file holds the pure core plus per-doc sidecar IO, intentionally free
 * of state.ts coupling so it unit-tests in isolation.
 */
...
// always stays. adr: adr/0006-document-history-attribution.md
