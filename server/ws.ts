...
import { switchDocument, createDocument, deleteDocument, getActiveFilename, promoteTempFile, listDocuments, acceptPendingTitle, rejectPendingTitle, getPendingTitle, collectAcceptedProvenance, appendProvenanceRecords } from './documents.js';
import { removeDocFromAllWorkspaces } from './workspaces.js';
import { commitFromFile, commitEvents } from './commits.js';
import { canonicalizeIdentifier, resolveDocPath } from './helpers.js';
import { isWsOriginAllowed } from './site-gate.js';
import { markdownToTiptap } from './markdown.js';
import { nodeTextPreview, diagLog, loadOverlay, liveOverlayEntries, applyOverlayPure } from './pending-overlay.js';
 ...
 *  if the doc has no pending nodes. adr: adr/0005-pending-overlay-model.md */
...
export function setupWebSocket(server: Server): void {
  const wss = new WebSocketServer({
    ...
    verifyClient: ({ req }: { req: import('http').IncomingMessage }) => {
      ...
      if (bearer && req.headers.authorization !== `Bearer ${bearer}`) return false;
      const origin = req.headers.origin;
      // No origin = non-browser client (MCP stdio / external agents) — allowed; the
      // bearer gate and the HTTP Host gate still bound them. With an origin: exact
      // site allowlist entry or loopback hostname (shared site-gate rules).
      if (!origin) return true;
      return isWsOriginAllowed(origin);
    },
  });

  // Spike patch #10: push commit events to connected clients.
  commitEvents.on('commit', (evt: { docId: string; trigger: string; ts: number }) => {
    const msg = JSON.stringify({ type: 'commit', ...evt });
  ...
  // The shelf moved (librarian sync, adopt, restore) — Library views refetch.
  onLibraryChanged(() => broadcastLibraryChanged());

  // A debounced save aimed at the read-only shelf is refused without throwing
  // (state.ts contains it so the timer callback can't crash the process) —
  // surface it, or the user's typing would silently go nowhere.
  onSaveRefused(() => {
    broadcastToast('Save refused — this file is in the read-only library shelf. Adopt a copy to edit it.', 'error');
  });
      ...
      try {
        ...
        if (msg.type === 'doc-update' && msg.document) {
          ...
          if (isAgentLocked(browserFilename || getActiveFilename())) {
            ...
            // preserved. adr: adr/0005-pending-overlay-model.md
        ...
        if (msg.type === 'title-update' && msg.title) {
          ...
          // diff UI dismisses. adr: adr/0005-pending-overlay-model.md
        ...
        if (msg.type === 'pending-resolved' && msg.filename) {
          ...
          if (action === 'reject' && isAgentStub(resolvedFilename)) {
            cancelDebouncedSave();
            try {
              // Membership follows success (F1 rider, spec 2026-09-13): if the
              // delete fails (e.g. only doc remaining), the catch falls through
              // to strip+save — membership must stay intact in that case.
          ...
          if (isActiveDoc) {
            ...
            // Provenance: read the author tag off each accepted pending entry
            // (atomic with the change) and append records to the frontmatter
            // provenance array. Same stamp the REST resolve-entry path applies
            // (documents.ts resolveOverlayEntry). Must run BEFORE
            // stripPendingAttrs/save clears the sidecar.
            if (action === 'accept') {
              // Orphan filter: the in-memory overlay may still carry entries whose
              // target node was externally removed and reloaded. applyOverlayPure
              // skips them when building the merged view, so the record must also
              // exclude them (audit index states only what was really accepted).
              const pendingEntries = liveOverlayEntries(getDocument().content ?? [], getOverlayEntries());
              const records = pendingEntries.length > 0
                ? collectAcceptedProvenance(pendingEntries, new Date().toISOString())
                : [];
              if (records.length > 0) setMetadata(appendProvenanceRecords(getMetadata(), records));
            }
            stripPendingAttrs();
            save();
            ...
            if (action === 'accept') unmarkAgentStub(resolvedFilename);
            // Provenance: merge the sidecar's agent provenance into
            // frontmatter via the single strip write (injectedMetadata) —
            // mirrors the REST resolve-entry path for non-active docs
            // (documents.ts resolveOverlayEntry). docId resolution order:
            // frontmatter, then the document cache — the same cached.docId
            // source applyChangesToFile used at propose time (lazy docId
            // migration means frontmatter may lag the cache). A cache-
            // recovered docId is persisted back so the file self-heals; a
            // total miss logs a warning and continues without provenance
            // (never silent).
            let injectedMetadata: Record<string, any> | undefined;
            if (action === 'accept') {
              try {
                const filePath = resolveDocPath(resolvedFilename);
                const raw = readFileSync(filePath, 'utf-8');
                const { data } = matter(raw);
                let docId = (data.docId && typeof data.docId === 'string') ? data.docId : '';
                ...
                if (!docId) {
                  console.warn(`[WS] pending-resolved accept: no docId for ${resolvedFilename} (frontmatter + cache) — provenance not stamped`);
                } else {
                  // Orphan filter: liveness against the merged view (canonical +
                  // sidecar), exactly like documents.ts resolveDocFile. Entries
                  // whose target nodeId is absent after overlay merge must not
                  // land in the provenance record.
                  const parsed = markdownToTiptap(raw);
                  const mergedDoc = applyOverlayPure(parsed.document, loadOverlay(docId));
                  const pendingEntries = liveOverlayEntries(mergedDoc.content ?? [], loadOverlay(docId));
                  const records = pendingEntries.length > 0
                    ? collectAcceptedProvenance(pendingEntries, new Date().toISOString())
                    : [];
                  injectedMetadata = {
                    ...(recoveredFromCache ? { docId } : {}),
                    ...(records.length > 0 ? { provenance: appendProvenanceRecords(data, records).provenance } : {}),
                  };
                }
              } catch (err: any) {
                console.warn(`[WS] pending-resolved accept: provenance stamp failed for ${resolvedFilename}: ${err?.message ?? err}`);
              }
            }
            stripPendingAttrsFromFile(resolvedFilename, action === 'accept', injectedMetadata);
          }
          broadcastPendingDocsChanged();
        }
...
export function broadcastChatProgress(docId: string, sessionId: string, line: string): void {
  const msg = JSON.stringify({ type: 'chat-progress', docId, sessionId, line });
  for (const ws of clients) {
    if (ws.readyState === WebSocket.OPEN) ws.send(msg);
  }
}
...
/** The shelf moved (librarian mirror write, or an adopt/restore) — Library
 *  views re-fetch the listing. Mirrors broadcastDocumentsChanged's shape;
 *  fired from the state library watcher and the adopt/restore routes. */
export function broadcastLibraryChanged(): void {
  const msg = JSON.stringify({ type: 'library-changed' });
  for (const ws of clients) {
    if (ws.readyState === WebSocket.OPEN) ws.send(msg);
  }
}
