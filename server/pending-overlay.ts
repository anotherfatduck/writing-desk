...
/** The provenance tag a propose call attaches to its changes (pending-side).
 *  Produced by the conductor's buildProvenance; validated by write_to_pad's
 *  zod schema. External writers may attach arbitrary shapes — the index
 *  signature keeps that runtime tolerance (no validation is added here);
 *  the four required fields are exactly the ones the zod schema requires.
 *  On accept, tags coalesce into ProvenanceRecord (documents.ts) — the
 *  typed accept-side boundary. */
...
export interface PendingEntry {
  ...
  // ---- Attribution (atomic with the change; stamped by propose_edits) ----
  /** Who proposed this change. Undefined = external/agent write with no
   *  provenance argument. Travels attrs → entry → frontmatter record on
   *  accept. adr: (per-change provenance — see spec 2026-09-08) */
  ...
  function walk(nodes: any[], parentNodeId: string | null): void {
    ...
    for (const node of nodes) {
      ...
      if (nodeId && status) {
        ...
        if (node.attrs.pendingProvenance != null) entry.provenance = node.attrs.pendingProvenance;
...
const PENDING_ATTR_KEYS = [
  'pendingStatus', 'pendingOriginalContent', 'pendingProvenance', 'pendingGroupId',
...
export function applyOverlay(canonical: any, entries: PendingEntry[]): ApplyResult {
  ...
  for (const entry of entries) {
    ...
    if (entry.provenance != null) target.attrs.pendingProvenance = entry.provenance;
  ...
  for (const entry of entries) {
    ...
    if (entry.provenance != null) newNode.attrs.pendingProvenance = entry.provenance;
...
export function applyOverlayPure(canonical: any, entries: PendingEntry[]): any {
  ...
  // are untouched. adr: adr/0005-pending-overlay-model.md
  ...
  // placement and would splice duplicate copies. adr: adr/0005-pending-overlay-model.md
  ...
  for (const entry of entries) {
    ...
    if (existing) {
      ...
      if (entry.provenance != null) existing.attrs.pendingProvenance = entry.provenance;
 ...
 *
 * adr: adr/0005-pending-overlay-model.md · adr: adr/tweet-paragraph-convention.md
 */
 ...
 * adr: adr/0005-pending-overlay-model.md
 */
/** Filter overlay entries to those whose target node still exists in the
 *  given merged doc content. A delete entry whose target was externally
 *  removed between propose and accept is discarded by the overlay merge —
 *  it must not land in a provenance record's accepted[] (the audit index
 *  states only what was really accepted). Compute BEFORE any accept/reject
 *  mutation touches the doc. */
export function liveOverlayEntries(docContent: any[], entries: PendingEntry[]): PendingEntry[] {
  const live = new Set<string>();
  (function walk(nodes: any[]) {
    for (const node of nodes ?? []) {
      if (node?.attrs?.id) live.add(node.attrs.id as string);
      if (node?.content) walk(node.content);
    }
  })(docContent);
  return entries.filter((e) => live.has(e.nodeId));
}
