...
import { existsSync, readFileSync, writeFileSync, unlinkSync, readdirSync, statSync, utimesSync, watch, mkdirSync, type Stats, type FSWatcher } from 'fs';
...
import { getDataDir, TEMP_PREFIX, ensureDataDir, filePathForTitle, tempFilePath, generateNodeId, LEAF_BLOCK_TYPES, resolveDocPath, isExternalDoc, atomicWriteFileSync, canonicalizePath, canonicalizeIdentifier, assertWritableDocPath, isLibraryReadOnlyError, ROOT_DIR, type CanonPath } from './helpers.js';
...
import { extractOverlay, applyOverlayPure, splitMergedDoc, reconcileCanonicalToBaselines, saveOverlay, loadOverlay, loadDocFromDisk, deleteOverlay, clearAllOverlays, migrateLegacyPending, repairOverlaysOnStartup, diagLog, type PendingEntry, type ProposedProvenance } from './pending-overlay.js';
...
export interface DocumentInfo {
  ...
  reviewGate?: { phase: 'submitted' | 'in-review' | 'published' | 'returned' | 'conflict'; since: string };
  /** The library adoption stamp (`<writerId>/<docId>`) — set on a desk doc that
   *  was checked out of the shelf by adopt; absent on ordinary docs. How the
   *  client knows a doc is an adopted library article (spec §2). Derived from
   *  frontmatter `library.key` by the same predicate as readLibraryKey. */
  libraryKey?: string | null;
  /** ISO timestamp stamped at adopt (library.adoptedAt); null pre-stamp. */
...
// ReadDirectoryChangesW on Windows. The watch is on the file's PARENT dir
// (filename-filtered) because all writes land via tmp+rename — a watch on
// the file inode goes deaf after the first rename-over. chokidar would add
// value for recursive directory trees, not here.
//
...
export function startActiveDocWatcher(): void {
  ...
  try {
    ...
    // Watch the parent DIRECTORY, not the file: every writer (this app's
    // atomicWriteFileSync, the orchestrator's review.* stamps) lands via
    // tmp+rename, which REPLACES the file's inode — a watch on the file
    // itself follows the old inode and goes deaf after the first
    // rename-over (UAT 2026-09-11: the gate never saw the orchestrator's
    // stamps). A dir watch survives every rename-over; the filename filter
    // keeps sibling files (.versions, other docs) from reloading us, and
    // the mtime check below still discards our own writes and no-op events.
    const watchedDir = dirname(state.filePath);
    const watchedBase = basename(state.filePath);
    activeWatcher = watch(watchedDir, { persistent: false }, (_eventType, filename) => {
      // filename can be null on some platforms — fall through and let the
      // mtime check decide; a false positive here is one cheap reload.
      if (filename !== null && filename !== undefined && filename !== watchedBase) return;
...
// ============================================================================
// LIBRARY SAVE REFUSALS — containing a non-request-context refusal
// ============================================================================
//
// The read-only chokepoint throws. Request contexts (HTTP routes, WS message
// handling) catch and answer with a 4xx; a save fired from the debounce timer
// has no caller to catch it, so it reports the refusal through this channel
// instead of escaping the callback (see the timer in `debouncedSave`).
// adr: adr/0007-git-orchestrator-dumb-host-side.md

export interface SaveRefused { filePath: string }
type SaveRefusedListener = (info: SaveRefused) => void;
const saveRefusedListeners: Set<SaveRefusedListener> = new Set();

/** Subscribe to contained save refusals (ws.ts surfaces them to the clients —
 *  a save that silently does nothing is worse than a visible one). */
export function onSaveRefused(listener: SaveRefusedListener): () => void {
  saveRefusedListeners.add(listener);
  return () => saveRefusedListeners.delete(listener);
}

function reportSaveRefused(filePath: string): void {
  console.warn(`[State] save refused — library area is read-only: ${filePath}`);
  for (const listener of saveRefusedListeners) {
    try { listener({ filePath }); }
    catch (err) { console.error('[State] save-refused listener threw:', err); }
  }
}

// ============================================================================
// LIBRARY WATCHER — the read-only shelf beneath ROOT_DIR/library/
// ============================================================================
//
// The librarian rewrites mirrors out-of-process (atomic tmp+rename, exactly
// like the orchestrator's review stamps), so we watch the shelf DIRECTORY for
// the same inode-survival reason the active-doc watcher does (adr: adr/0004) —
// with a filename filter, an 80ms burst debounce, and a signature compare so a
// no-op event never wakes the clients. The listener set feeds the WS
// `library-changed` push, which tells Library views to re-fetch the listing.
// adr: adr/0007-git-orchestrator-dumb-host-side.md

type LibraryChangedListener = () => void;
const libraryChangedListeners: Set<LibraryChangedListener> = new Set();

/** Register a library-changed listener (ws.ts pushes the broadcast). Returns
 *  an unsubscribe — same shape as onDocumentReloaded/onChanges. */
...
function notifyLibraryChanged(): void {
  for (const listener of libraryChangedListeners) {
    try { listener(); }
    catch (err) { console.error('[State] library-changed listener threw:', err); }
  }
}

let libraryWatcher: FSWatcher | null = null;
let libraryDebounceTimer: ReturnType<typeof setTimeout> | null = null;
let librarySnapshot = '';

/** The shelf's whole-state fingerprint: every mirror + manifest's
 *  name/size/mtime, sorted so it is order-stable. Cheap (one readdir + stats)
 *  and enough to tell a real change from a spurious fs event. */
function librarySignature(libDir: string): string {
  try {
    return readdirSync(libDir)
      ...
      .map((f) => {
        try {
          const s = statSync(join(libDir, f));
...
/** Tear down the library watcher (if any) and clear bookkeeping. */
function stopLibraryWatcher(): void {
  if (libraryDebounceTimer) {
    clearTimeout(libraryDebounceTimer);
    libraryDebounceTimer = null;
  }
  if (libraryWatcher) {
    try { libraryWatcher.close(); } catch { /* best-effort */ }
    libraryWatcher = null;
  }
}

/** Handle a shelf event after debounce — notify only when the fingerprint
 *  actually moved (drops no-op events and any churn the app itself caused). */
function handleLibraryWatcherEvent(): void {
  const libDir = join(ROOT_DIR, 'library');
  const next = librarySignature(libDir);
  if (next === librarySnapshot) return;
  librarySnapshot = next;
  notifyLibraryChanged();
}

/** Start the shelf watcher. Called once from load(), alongside
 *  startActiveDocWatcher(). Creates the shelf directory when absent so the
 *  watch can attach before the librarian's first run. */
export function startLibraryWatcher(): void {
  stopLibraryWatcher();
  const libDir = join(ROOT_DIR, 'library');
  try {
    mkdirSync(libDir, { recursive: true });
    librarySnapshot = librarySignature(libDir);
    libraryWatcher = watch(libDir, { persistent: false }, (_eventType, filename) => {
      // filename can be null on some platforms — fall through and let the
      // signature compare decide; a false positive is one cheap re-readdir.
      if (filename !== null && filename !== undefined && !/\.(md|json)$/.test(filename)) return;
      if (libraryDebounceTimer) clearTimeout(libraryDebounceTimer);
      libraryDebounceTimer = setTimeout(handleLibraryWatcherEvent, 80);
    });
    libraryWatcher.on('error', (err) => {
      console.error('[State] library watcher error:', err.message);
      stopLibraryWatcher();
    });
  } catch (err: any) {
    // Degraded, not broken: without a watcher the Library view simply relies
    // on its own refetch. Mirrors the active-doc watcher's tolerance.
    console.error(`[State] failed to watch ${libDir}:`, err?.message || err);
    stopLibraryWatcher();
  }
}
...
/** Strip the legacy `pending` frontmatter field. The overlay sidecar is now
 *  the authoritative pending store; any in-doc `pending:` that survives from a
 *  pre-migration file must be dropped on first save so external editors never
 *  see stale overlay data in the .md frontmatter. */
function stripLegacyPending(metadata: Record<string, any>): void {
  if (metadata && 'pending' in metadata) delete metadata.pending;
}
  ...
  function collectPending(nodes: any[]) {
    ...
    for (const node of nodes) {
      if (node.attrs?.pendingStatus && node.attrs?.id) {
        ...
        // Copy all pending-related attrs if present. Provenance is stored under
        // the attr name in the transfer map so applyPending writes it back as
        // pendingProvenance (not bare provenance), matching the attr contract.
        if (node.attrs.pendingOriginalContent != null) entry.pendingOriginalContent = node.attrs.pendingOriginalContent;
        if (node.attrs.pendingProvenance != null) entry.pendingProvenance = node.attrs.pendingProvenance;
...
// adr: adr/0006-document-history-attribution.md (invariant 3 — no module-global shim;
...
export function debouncedSave(actor: Actor = 'human'): void {
  ...
  saveTimer = setTimeout(() => {
    ...
    // A throw escaping a timer callback is an uncaughtException, and
    // bin/server.ts exits(1) on those — so a doc whose path is the read-only
    // shelf would crashloop the server the moment the user typed (open the
    // mirror → WS doc-update → debounce → save() → refusal). Contain the
    // library refusal and report it; every other failure keeps today's
    // fail-fast behaviour.
    try {
      save(a);
    } catch (err) {
      if (!isLibraryReadOnlyError(err)) throw err;
      reportSaveRefused(state.filePath);
    }
  }, SAVE_DEBOUNCE_MS);
}
...
// ---- Quiesce (spike patch #9): orchestrator checkout window ----
// Flush any armed debounce NOW (under its own actor — nothing is lost), then
// freeze saves until unquiesce or expiry. The orchestrator sequence is
// quiesce → git ops on the workspace → unquiesce; a save that arrives mid-
// window is refused (logged, not silent) rather than racing the branch swap.
let quiescedUntil = 0;

export function quiesce(ms: number): number {
  const until = Date.now() + Math.max(0, Math.min(ms, 60_000));
  if (saveTimer) {
    clearTimeout(saveTimer);
    saveTimer = null;
    const a = pendingSaveActor ?? 'human';
...
export function unquiesce(): void { quiescedUntil = 0; }

export function isQuiesced(): boolean { return Date.now() < quiescedUntil; }

export function applyChanges(changes: NodeChange[], provenance?: ProposedProvenance): { count: number; lastNodeId: string | null } {
  ...
  // vanishes despite a success response. adr: adr/0005-pending-overlay-model.md
  const version = bumpDocVersion();
  setAgentLockActive();

  // Apply to server-side document (source of truth)
  const processed = applyChangesToDocument(changes, provenance);
 ...
 */
function applyChangesToDoc(doc: PadDocument, changes: NodeChange[], autoAccept: boolean = false, provenance?: ProposedProvenance): NodeChange[] {
  const processed: NodeChange[] = [];

  // Auto-clean: if the TARGET doc has only a single empty paragraph and the
  // first change is an insert, convert to a rewrite so the empty node gets
  // replaced silently (shows as a green insert decoration, not a red delete).
  // Keyed off the doc being applied to — NEVER the active doc. Keying off the
  // active doc converted proposes to non-active fresh docs into rewrites of
  // node IDs absent in the target, silently dropping them (appliedCount 0).
  // Runs against `doc` AFTER resolution (cache or disk load), so the empty
  // node's ID is the same one the arms below will match.
  const emptyFirst = doc.content?.length === 1 ? doc.content[0] : null;
  if (
    emptyFirst && emptyFirst.type === 'paragraph' &&
    (!emptyFirst.content || emptyFirst.content.length === 0) && emptyFirst.attrs?.id
  ) {
    const insertIdx = changes.findIndex((c: any) => c.operation === 'insert');
    if (insertIdx !== -1) {
      changes = changes.slice();
      changes[insertIdx] = { ...changes[insertIdx], operation: 'rewrite', nodeId: emptyFirst.attrs.id };
      delete (changes[insertIdx] as any).afterNodeId;
    }
  }
  ...
  for (const change of changes) {
    if (change.operation === 'rewrite' && change.nodeId && change.content) {
      ...
      if (isWrappedRewrite && !autoAccept) {
        ...
        const innerWithPending = {
          ...
          attrs: {
            ...
            ...(provenance ? { pendingProvenance: provenance } : {}),
            ...(partialRange ? {
      ...
      if (!autoAccept) markLeafBlocksAsPending(extraNodes, 'insert', provenance);
      ...
      if (!autoAccept) markLeafBlocksAsPending(contentWithIds, 'insert', provenance);
...
function applyChangesToDocument(changes: NodeChange[], provenance?: ProposedProvenance): NodeChange[] {
  const autoAccept = isAutoAcceptActive(activeDocFilename(), state.metadata);
  const processed = applyChangesToDoc(state.document, changes, autoAccept, provenance);
...
export function setActiveDocument(
  ...
  // staged title rename survives doc-switch and restart. adr: adr/0005-pending-overlay-model.md
...
/** Remove specific entries from the active document's overlay by nodeId.
 *  Does NOT recompute the merged view — callers that have already mutated
 *  state.document in place use this to keep state.overlay consistent before
 *  calling save(). */
export function removeOverlayEntries(nodeIds: string[]): void {
  for (const id of nodeIds) state.overlay.delete(id);
}
...
export function reloadActiveDocFromDisk(): {
  ...
  if (sidecar.length > 0) {
    ...
    // Drop the dead in-frontmatter `pending:` key so the rewrite below writes
    // canonical-only metadata. writeToDisk already strips it at serialization,
    // but removing it from memory keeps the rest of the session consistent.
    stripLegacyPending(state.metadata);
  } else {
    entries = [];
  }
  ...
  // The reload path refreshes disk frontmatter with an atomic write, which on
  // Linux/macOS renames a temp file over the watched inode. fs.watch is tied to
  // the old inode, so the existing handle stops receiving events for the new
  // file. Restart the watcher so subsequent external writes keep flowing.
  // adr: adr/0004-active-doc-watcher.md
  startActiveDocWatcher();
...
function mergeOverlayOnLoad(): PendingEntry[] {
  ...
  if (sidecar.length > 0) {
    ...
    // One-time migration: the incoming doc carried pending attrs (legacy
    // frontmatter `pending:` rehydrated by the parser, or a merged doc passed
    // in directly). The sidecar now owns them. Bump docVersion so the next
    // save cannot short-circuit at the no-op gate — without the bump, an
    // existing file keeps its stale `pending:` frontmatter on disk forever.
    // `tiptapToMarkdown` strips any remaining `pending` key at serialization.
    stripLegacyPending(state.metadata);
    bumpDocVersion();
    return legacy;
  }
...
export function cloneWithPendingReverted(doc: PadDocument): PadDocument {
  const PENDING_KEYS = ['pendingStatus', 'pendingOriginalContent', 'pendingProvenance', 'pendingGroupId', 'pendingTextEdits', 'pendingSelectionFrom', 'pendingSelectionTo', 'pendingOriginalFrom', 'pendingOriginalTo', 'pendingOrphan', 'pendingStaleBaseline'];
 ...
 */
function markLeafBlocksAsPending(nodes: any[], status: string, provenance?: ProposedProvenance): void {
  if (!nodes) return;
  for (const node of nodes) {
    if (node.type && LEAF_BLOCK_TYPES.has(node.type)) {
      node.attrs = { ...node.attrs, pendingStatus: status, ...(provenance ? { pendingProvenance: provenance } : {}) };
      if (!node.attrs.id) {
        node.attrs.id = generateNodeId();
      }
    } else if (node.content) {
      markLeafBlocksAsPending(node.content, status, provenance);
    }
  }
}
...
function writeToDisk(actor: Actor = 'human'): void {
  // Library read-only chokepoint (spec §3): `state.filePath` may be an external
  // doc opened from anywhere on disk, so a save aimed at the shelf is refused
  // here first. This is the single chokepoint every save funnels through — the
  // HTTP save route, the WS `save` client message, and flush — so the guard
  // covers them all. adr: adr/0007-git-orchestrator-dumb-host-side.md
  assertWritableDocPath(state.filePath);
  ...
  if (isExternalDoc(state.filePath)) {
    ...
    //
    // adr: adr/node-identity-matcher.md · adr: adr/0005-pending-overlay-model.md
    ...
    if (state.docId) {
      try {
        ...
        // all write tools fire it. adr: adr/0006-document-history-attribution.md
...
export function save(actor: Actor = 'human'): void {
  if (isQuiesced()) {
    console.warn('[quiesce] save refused while quiesced (orchestrator owns the tree)');
    return;
  }
  ...
  // attribution spans anyway). adr: adr/0006-document-history-attribution.md
...
export function load(): void {
  ...
  // loop reads deduped sidecars, not corrupted ones. adr: adr/0005-pending-overlay-model.md
  ...
  // unified reload pathway. adr: adr/0004-active-doc-watcher.md
  startActiveDocWatcher();

  // Subscribe the shelf watcher so librarian mirror writes push
  // library-changed to connected clients. adr: adr/0007.
  startLibraryWatcher();
...
export function saveDocToFile(filename: string, doc: PadDocument): void {
  ...
  try {
    ...
    if (docId) {
      ...
      // the human's. Snapshot + attribute as 'human'. adr: adr/0006-document-history-attribution.md
      if (!isExternalDoc(targetPath)) {
        let snapshotTs: number | null = null;
 ...
 * adr: adr/agent-stub-model.md
 */
export function stripPendingAttrsFromFile(
  filename: string,
  _legacyClearAgentCreated?: boolean,
  ...
  try {
    ...
    function strip(nodes: any[]) {
      ...
      for (const node of nodes) {
        if (node.attrs?.pendingStatus) {
          ...
          delete node.attrs.pendingProvenance;
    ...
    // Injected metadata (provenance / cache-recovered docId from the WS
    // non-active accept path) merges into frontmatter on this same atomic
    // write — folding what used to be a separate pre-write into one.
    const mergedMetadata = injectedMetadata ? { ...parsed.metadata, ...injectedMetadata } : parsed.metadata;
    let markdown: string;
    if (isExternalDoc(targetPath)) {
      // External docs keep their raw frontmatter verbatim — injected
      // metadata is not merged (rawFrontmatter preservation contract).
      const body = tiptapToBody(parsed.document);
      markdown = parsed.rawFrontmatter
        ? `---\n${parsed.rawFrontmatter}\n---\n\n${body}`
        : body;
    } else {
      markdown = tiptapToMarkdown(parsed.document, parsed.title, mergedMetadata);
    }
    atomicWriteFileSync(targetPath, markdown);
    // Pending was just cleared on disk; the sidecar overlay must go too,
    // otherwise the next load would re-apply stale pending entries. The
    // resolved docId prefers frontmatter, then the injected value — the
    // WS accept path recovers docId from the cache when frontmatter lacks
    // it (same defect class as the provenance gate).
    // adr: adr/0005-pending-overlay-model.md
    const docId = (mergedMetadata && typeof mergedMetadata.docId === 'string') ? mergedMetadata.docId : '';
...
export function populateDocumentFile(filename: string, doc: PadDocument): { title: string; wordCount: number; pendingCount: number } {
  ...
  // subsequent flushDocToFile. adr: adr/0005-pending-overlay-model.md
  ...
  // mcp.ts:populate_document. adr: adr/0005-pending-overlay-model.md
 ...
 */
export function applyChangesToFile(filename: string, changes: NodeChange[], provenance?: ProposedProvenance): { count: number; lastNodeId: string | null } {
  const targetPath = resolveDocPath(filename);
  ...
  const processed = applyChangesToDoc(doc, changes, autoAccept, provenance);
  if (processed.length > 0) {
...
export function applyTextEditsToFile(filename: string, nodeId: string, edits: TextEdit[]): { success: boolean; error?: string } {
  ...
  if (cached) {
    ...
    // findNode call below would fail. adr: adr/0005-pending-overlay-model.md
    const loaded = loadDocFromDisk(filename);
    doc = loaded.document;
    title = loaded.title;
    metadata = loaded.metadata;
